component extends="wheels.tests.Test" {
	...
	function test_migrateto_migrate_up_from_0_to_001() {
		...
		info = $dbinfo(datasource = application.wheels.dataSourceName, type = "tables", pattern = "c_o_r_e_bunyips");
	...
	function test_migrateto_migrate_up_from_0_to_003() {
		...
		info1 = $dbinfo(datasource = application.wheels.dataSourceName, type = "tables", pattern = "c_o_r_e_bunyips");
		info2 = $dbinfo(datasource = application.wheels.dataSourceName, type = "tables", pattern = "c_o_r_e_dropbears");
		info3 = $dbinfo(datasource = application.wheels.dataSourceName, type = "tables", pattern = "c_o_r_e_hoopsnakes");
		actual1 = ValueList(info1.table_name);
		actual2 = ValueList(info2.table_name);
		actual3 = ValueList(info3.table_name);

		assert("ListFindNoCase(actual1, 'c_o_r_e_bunyips')");
		assert("ListFindNoCase(actual2, 'c_o_r_e_dropbears')");
		assert("ListFindNoCase(actual3, 'c_o_r_e_hoopsnakes')");
	}
