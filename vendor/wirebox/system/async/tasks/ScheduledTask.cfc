...
component accessors="true" {
	...
	ScheduledTask function init(
		...
		// Utility class
		variables.util             = new wirebox.system.core.util.Util();
		// Link up the executor and name
		variables.executor         = arguments.executor;
		variables.name             = arguments.name;
		variables.group            = arguments.group;
		// time unit helper
		variables.dateTimeHelper   = new wirebox.system.async.time.DateTimeHelper();
		variables.timeUnitHelper   = new wirebox.system.async.time.TimeUnit();
		// Init Properties
		...
		variables.stats            = {
			...
			"lastResult"        : new wirebox.system.async.cbproxies.models.Optional(),
