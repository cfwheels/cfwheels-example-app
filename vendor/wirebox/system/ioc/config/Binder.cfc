...
component accessors="true" {
	 ...
	 *
	 * @return wirebox.system.ioc.config.Mapping
	 *
	...
	Binder function map( required alias, boolean force = false ){
		...
		variables.mappings[ name ] = new wirebox.system.ioc.config.Mapping( name );
	...
	/**
	 * Map to a provider object that must implement wirebox.system.ioc.IProvider or a closure or UDF
	 *
	 ...
	 * @classNamespace The package namespace to use for creating or connecting to CacheBox. Defaults to: wirebox.system.cache
	 */
	 ...
	 * @path       The path to the CFC that implements this scope, it must have an init() method and implement: wirebox.system.ioc.scopes.IScope
	 */
	 ...
	 *
	 * @return wirebox.system.aop.Matcher
	 */
	function match(){
		return new wirebox.system.aop.Matcher();
	}
