...
component accessors=true serializable=false {
	...
	/**
	 * Get a reference to a registered cache in this factory.  If the cache does not exist it will return an exception. Type: wirebox.system.cache.providers.ICacheProvider
	 *
	...
	/**
	 * Get the default cache provider of type wirebox.system.cache.providers.ICacheProvider"
	 *
	 * @return wirebox.system.cache.providers.ICacheProvider
	 */
	...
	private CacheFactory function doScopeRegistration(){
		var scopeInfo = variables.config.getScopeRegistration();
		new wirebox.system.core.collections.ScopeStorage().put( scopeInfo.key, this, scopeInfo.scope );
		return this;
	}
