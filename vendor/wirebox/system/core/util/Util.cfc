...
component {
	...
	boolean function isFamilyType( required family, required target ){
		...
		switch ( arguments.family ) {
			case "handler": {
				familyPath = "wirebox.system.EventHandler";
				break;
			}
			case "interceptor": {
				familyPath = "wirebox.system.Interceptor";
				break;
			}
			default: {
	...
	/**
	 * @deprecated Refactor to use the Env Delegate: wirebox.system.core.delegates.Env
	 */
	function getSystemSetting( required key, defaultValue ){
		return new wirebox.system.core.delegates.Env().getSystemSetting( argumentCollection = arguments );
	}
	...
	function getSystemProperty( required key, defaultValue ){
		return new wirebox.system.core.delegates.Env().getSystemProperty( argumentCollection = arguments );
	}
	...
	function getEnv( required key, defaultValue ){
		return new wirebox.system.core.delegates.Env().getEnv( argumentCollection = arguments );
	}
